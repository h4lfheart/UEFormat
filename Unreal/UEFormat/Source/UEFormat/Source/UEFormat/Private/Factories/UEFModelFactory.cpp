...
void UEFModelFactory::SetMeshAttributes(FMeshDescription& MeshDesc, FLODData& Data)
{
	...
	//Vertices
	for (auto i = 0; i < Data.Vertices.Num(); ++i)
		VertexPositions.Set(i, FVector3f(Data.Vertices[i].X, -Data.Vertices[i].Y, Data.Vertices[i].Z));
	...
	for (auto i = 0; i < Data.Indices.Num(); i++) {
		...
		if (Data.Tangents.Num() > 0)
			VertexInstanceTangents.Set(i, FVector3f(Data.Tangents[Index].X, -Data.Tangents[Index].Y, Data.Tangents[Index].Z));
...
USkeleton* UEFModelFactory::CreateSkeleton(FString Name, UObject* Parent, EObjectFlags Flags, FSkeletonData& Data, FReferenceSkeleton& RefSkeleton)
{
	...
	for (const auto& Bone : Data.Bones)
	{
		...
		auto Location = FVector(Bone.BonePos.X, -Bone.BonePos.Y, Bone.BonePos.Z);
		Transform.SetLocation(Location);
		auto Rotation = FQuat(Bone.BoneRot.X, -Bone.BoneRot.Y, Bone.BoneRot.Z, -Bone.BoneRot.W);
	...
	for (const auto& Socket : Data.Sockets)
	{
		...
		NewSocket->RelativeLocation = FVector(Socket.SocketPos.X, -Socket.SocketPos.Y, Socket.SocketPos.Z);
		NewSocket->RelativeRotation = FQuat(Socket.SocketRot.X, -Socket.SocketRot.Y, Socket.SocketRot.Z, -Socket.SocketRot.W).Rotator();
